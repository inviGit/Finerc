...
fun AllTransactionsScreen(
    ...
    val txnCount = displayedList.size

    LaunchedEffect(uiState.transactions) {
        try {
            Log.d("AllTransactionScreen", "Successfully loaded $txnCount transactions")
        } catch (e: Exception) {
            Log.e("AllTransactionScreen", "Error loading transactions", e)
        }
    }

    // Derived facets from allTransactions
    val allYears by remember(uiState.transactions) {
        mutableStateOf(
            uiState.transactions.map {
                val c = Calendar.getInstance().apply { timeInMillis = it.txnDate }
                c.get(Calendar.YEAR)
            }.distinct().sortedDescending()
        )
    }
    // Month values as 1..12 paired to labels
    val allMonths by remember(uiState.transactions) {
        mutableStateOf(
            uiState.transactions.map {
                val c = Calendar.getInstance().apply { timeInMillis = it.txnDate }
    ...
    val allCategories by remember(uiState.transactions) {
        mutableStateOf(uiState.transactions.map { it.category }.distinct())
    }
    val allPlaces by remember(uiState.transactions) {
        mutableStateOf(
            uiState.transactions.map { it.place }.filter { it.isNotBlank() }.distinct()
                .sorted()
        )
    }
    val allBanks by remember(uiState.transactions) {
        mutableStateOf(
            uiState.transactions.map { it.bankName }.filter { it.isNotBlank() }.distinct()
                .sorted()
        )
    }
    ...
    Scaffold(
        ...
        topBar = {
            Column(
                ...
                val totalSpent = displayedList.sumOf { it.amount }
                val avgTransaction =
                    if (displayedList.isNotEmpty()) totalSpent / displayedList.size else 0.0
        ...
        LazyColumn(
            ...
            if (uiState.isLoading) {
                ...
                item { ErrorCard(error = "Error: ${uiState.error}") }
            } else if (displayedList.isEmpty()) {
                item { EmptyStateCard() }
            } else {
                items(
                    displayedList,
                    key = { it.id ?: "${it.id}-${it.txnDate}" }) { message ->
    ...
    if (isFilterSheetOpen) {
        ModalBottomSheet(
            ...
            Column(
                ...
                val maxAmount = remember(uiState.transactions) {
                    (uiState.transactions.maxOfOrNull { it.amount } ?: 0.0).toFloat()
                        .coerceAtLeast(1000f)
                }
                Slider(
...
fun applyFilterToTransaction(
    allTransactions: List<TransactionUiModel>,
    ...
    val filteredList = allTransactions.asSequence().filter { msg ->
        val cal = Calendar.getInstance().apply { timeInMillis = msg.txnDate }
        val yearOk =
    ...
    return when (selectedSortIndex) {
        0 -> filteredList.sortedByDescending { it.txnDate }
        1 -> filteredList.sortedBy { it.txnDate }
